...
import bj.anydef.tls.cert.extensions.canParseAsIpAddress
import bj.anydef.tls.cert.extensions.toCanonicalHost
import printlog
...
object AnydefHostnameVerifier : HostnameVerifier {
  ...
  override fun verify(host: String, session: SSLSession): Boolean {
    return try {
      if (verify(host, session.peerCertificates[0] as X509Certificate)) {
        true
      } else {
        printlog("host[$host] verify failed ")
        false
      }
